...
public class SingletonPattern {
    ...
    public static void main(String[] args) throws NoSuchMethodException {
        ...
        checkThat("getInstance must not return null", "first != null", first != null);
        checkThat("both calls must return the same object", "first == second", first == second);
        first.setMessage("shared");
        check("singleton state must be shared", "second.getMessage()", "shared", second.getMessage());
        checkThat("constructor must remain private", "Modifier.isPrivate(Singleton.class.getDeclaredConstructor().getModifiers())", Modifier.isPrivate(Singleton.class.getDeclaredConstructor().getModifiers()));
        report("Challenge 58");
    }
