...
**Why this exists:** "what's new in Java 8?" is a panel certainty, and
every follow-up — lambdas, streams, `Optional` — hangs off it. This is
the Tier-1 study block: Java 8 built from nothing, assuming you have
never used these features. The chronological spine that places 8 among
the other releases is [java-versions.md](java-versions.md); the spoken
rapid-fire layer is
[core-java.md §4](core-java.md#4-java-8-know-it-cold), which links
into the sections here.

**How to drill:** read §A→§I in order to *understand* — each section
assumes the one before it. Replay the analogy card to *recall*. Then
drill blind off [core-java.md §4](core-java.md#4-java-8-know-it-cold)
to *load*.
