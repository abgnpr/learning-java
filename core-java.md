...
**Read in order — each answer assumes the one above it:**

```mermaid
flowchart TD
    subgraph problems ["the two problems"]
        direction LR
        R["race condition<br/>(atomicity)"]
        VP["visibility problem<br/>(happens-before)"]
    end

    R --> CS["critical section<br/>(the region to protect)"]
    VP --> CS

    subgraph fixes ["the fixes — in dependency order"]
        direction LR
        SY["synchronized<br/>(mutex + visibility)"] --> WN["wait / notify"]
        WN --> RL["ReentrantLock"] --> CO["Condition"] --> VO["volatile"]
    end

    CS --> SY

    subgraph running ["running the work"]
        direction LR
        EX["executors"] --> VT["virtual threads"]
    end

    VO --> EX
    VT --> CC["concurrent collections<br/>(don't hand-roll it)"]
```

Jumping in at `ReentrantLock` is what makes this topic feel
impossible: every one of its extras is defined against something
earlier. (`monitor` is the one exception — defined up front,
because the thread-state table needs it.)
...
- **Race condition?** *(start here — the problem every tool below
  exists to solve)* — two threads touch the same shared state and
  the result depends on who wins the interleaving.
...
- **Critical section?** — the specific lines that touch shared
  ...
  - **mutex** ("mutual exclusion") = the mechanism that admits one
    ...
    `synchronized` (next), or a `Lock` (further down).
...
- **synchronized?** — mutual exclusion: only one thread at a time
  ...
  **A lock is two guarantees, not one.** This is the step most
  ...
  they see*. It fixes both problems above at once.
  ...
  - contrast to keep: `volatile` gives *only* the visibility half,
    ...
    broken on a volatile field *(below)*
...
- **synchronized vs Lock (ReentrantLock)?** *(X-vs-Y staple)*
  ...
  *(Beginner trap: calling `tryLock()` bare, outside an `if`, then
  ...
  below: a thread that can't get the *second* lock backs off and
  retries instead of hanging on it forever.
  ...
  **Verb 4 — wake precisely.** A monitor has one wait-set, so
  `notifyAll()` wakes producers *and* consumers and most go
  straight back to waiting. A lock gives each role its own queue:
  ...
  **The `synchronized` way** — one wait-set, so every wake is a
  ...
  **The `Condition` way** — one queue per role, so you wake only
  the side that can actually move:
  ...
  - `while`, never `if`: a woken thread must re-check, because it
    ...
    changed again (and spurious wakeups are legal)
...
- **Concurrent collections?** *(reach for these before you write a
  ...
  - `Collections.synchronizedMap(m)` is the *old* fix: one lock
    ...
    difference. (§2 has the full comparison.)
